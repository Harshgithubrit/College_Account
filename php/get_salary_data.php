...
if (isset($_REQUEST['page_no']) && $_REQUEST['page_no']!="") {
    ...
    //  $query=html_entity_decode($_REQUEST['query']);
    

	//  $Total_record_query=html_entity_decode($_REQUEST['Total_record_query']);
    $query= "select *,ROUND(monthly_salary/working_days*(working_days-absent_days)*(PF+ESI)/100) as total_deduction from employee_salary where date between '$fdate' and '$tdate' and (employee_id LIKE '%$ser%' or employee_name LIKE '%$ser%' or pay_slip_id LIKE '%$ser%') and status='1' order by id DESC " ; 
   
   
    // $query =  "select * from employee where user_id LIKE '%$ser%' order by id DESC ";

    $Total_record_query="SELECT COUNT(*) As total_records FROM `employee_salary` where employee_name LIKE'%$ser%' and  (employee_id LIKE '%$ser%' or employee_name LIKE '%$ser%' or pay_slip_id LIKE '%$ser%') and date between '$fdate' and '$tdate'  and status='1' ";
