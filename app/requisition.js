...
if( $scope.category == null ||  $scope.category == undefined ||  $scope.category == "" ){
   ...
   // var fee_date= convert($scope.requisition_date);
    swal("Loading.....");
       ...
       $http.post("php/requisition.php?user_id="+ EMP_id 
...
//   $http.post("php/get_Requi_data_all.php?uid="+EMP_id)
//       .then(function (result) {
//       // console.log(result.data);
...
var query_item="select  sum(total) as total, sum(quantity) as quantity from account_requisition_item   ";

var query="select  * from account_requisition  group by receipt_no order by id DESC ";
  var Total_record_query="SELECT COUNT(*) As total_records FROM `account_requisition` ";
  var records_per_page="10";
  console.log(query);

  
    $http.get("php/get_Requi_data_all_pagination.php?records_per_page="+records_per_page+"&page_no="+page_no+"&query="+query+"&Total_record_query="+Total_record_query+"&uid="+EMP_id+"&query_item="+query_item)
...
if(response.data.datastatus != "0") {
    ...
    $scope.days= response.data.tbldata.days;
...
$scope.preview_pdf = function(){ 
  ...
  //  alert(receipt_pdf_id);    
       window.open('export_pdf/examples/invoice.php?receipt_no='+receipt_no_last,'_blank');
  ...
  //  alert(receipt_pdf_id);    
       window.open('template/requisition.php?receipt_no='+receipt_no_last,'_blank');
...
swal.close();
 ...
 $scope.Requisition_Dete = function (index) {
  ...
  .then((willDelete) => {
    ...
    var  query = " delete from `account_requisition_item` where user_id='"+$scope.Requi_item_details[index].user_id+"' and receipt_no='"+$scope.Requi_item_details[index].receipt_no+"' and  id="+$scope.Requi_item_details[index].id+" ";
...
$scope.approve_data= function(){
  ...
  var query="update account_requisition set admin_remarks='"+$scope.approve_remarks+"', admin_approved='"+remarks_status+"', admin_approved_by ='"+EMP_id+"',hold_status='2',hold_date='"+dates+"' where receipt_no='"+receipt_no_last+"' ";
...
if(result.data['0'].status == "1") {
  ...
  // $http.post("php/Send_Mail_Requi_Approve.php?receipt_no="+receipt_no_last)
  // .then(function (result) {
  //   console.log(result);
  // }, function(result) {
  //     console.log(result);
  ...
  .then((willDelete) => {
    if (willDelete) {
      ...
      mnth = ("0" + (date.getMonth() + 1)).slice(-2),
        day = ("0" + date.getDate()).slice(-2);
      var dates = [date.getFullYear(), mnth, day].join("-");
  var query="update account_requisition set admin_remarks='"+$scope.approve_remarks+"', admin_approved='"+remarks_status+"', admin_approved_by ='"+EMP_id+"',hold_status='0.5',hold_date='"+dates+"' where receipt_no='"+receipt_no_last+"' ";
...
$scope.revert_data= function(){
  ...
  var query="update account_requisition set admin_remarks='"+$scope.approve_remarks+"', admin_approved='"+remarks_status+"', admin_approved_by ='"+EMP_id+"',hold_status='0' where receipt_no='"+receipt_no_last+"' ";
...
if(result.data['0'].status == "1") {
 ...
 swal("Also quotation list created successfully!");
...
$scope.Revert_remarks_sts= function(){
  remarks_status = "Reverted";
}
...
$scope.Req_Update = function () {
  ...
  if( $scope.category == null ||  $scope.category == undefined ||  $scope.category == "" ){
    ...
    var query="update account_requisition_item set item='"+$scope.item_nameE+"', item_description='"+$scope.item_descriptionE+"', quantity='"+$scope.quantityE+"',price='"+$scope.price_amountE+"',total='"+tot+"' where id="+uid+" and receipt_no='"+ureceipt_no+"' ";
...
function get_Requi_item_data(){ 
  ...
  $http.post("php/get_Requi_item_data.php?uid="+receipt_no_last)
      .then(function (result) {
      //  console.log(result.data);
if(result.data.status != "0") {
...
swal.close();
 ...
 function notification_send(rid){ 
   ...
   console.log("notification_send ", rid);
      ...
      $http.post("http://mjge.in/account/App_API/notification.php?recipet_id="+rid)
          .then(function (result) {
