...
var query="select * from account_requisition as a where a.user_id='"+EMP_id+"' group by a.receipt_no order by a.id DESC ";
console.log(query);  
...
$scope.submit_Rq= function(){
  ...
  .then((willDelete) => {
    ...
    var  query = " update `account_requisition`  set submitted ='YES',  admin_remarks='', admin_approved='Pending', account_approved_by='"+EMP_id+"',hold_status='1' where receipt_no='"+receipt_no_last+"' and  user_id='"+EMP_id+"' ";
...
$scope.GOBACK = function(){
    ...
    }else if(profile_type =="Principal" ) {
            window.opener = self;
    window.close();
