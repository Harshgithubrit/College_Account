...
var receipt_no_last="";
 ...
 var query="select verify_status, vendor_id, vendor_name, mobile, email, user_name, user_id, date, time, remarks, category, priority, receipt_no, admin_approved, admin_remarks,vendor_receipt_file,vendor_receipt_no from account_purchase_order  as a   group by a.receipt_no order by a.id DESC ";
...
// });
  ...
  $scope.GRN_Submit = function(){
   ...
   get_Requi_data(); notification_send(receipt_no_last);
...
$scope.print_cheque_axis=function()
{

  var paisa=  RsPaise(Math.round(document.getElementById('paid_amount').value*100)/100);

  localStorage.setItem("name", $scope.payble_name);
  localStorage.setItem("amt", $scope.paid_amount);
  localStorage.setItem("amtw", paisa);

  var url="php/print_axis.php?name="+$scope.payble_name+"&amtw="+paisa+"&amt="+$scope.paid_amount+"&date="+Newconvert($scope.cheque_date);
  window.open(url,'_blank');
  window.open(url);
...
$scope.print_cheque_uco=function()
{
  ...
  var url="php/print_uco.php?name="+$scope.payble_name+"&amtw="+paisa+"&amt="+$scope.paid_amount+"&date="+Newconvert($scope.cheque_date);
  window.open(url,'_blank');
  window.open(url);
