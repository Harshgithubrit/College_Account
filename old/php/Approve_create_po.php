...
if($result1->num_rows > 0) {
    ...
    if(mysqli_affected_rows($mysqli)>0){
if($user_full_name='Admin MJGE College'){
       $queryAppi="update account_quotation set director_status='0', admin_remarks='$approve_remarks', admin_approved='$remarks_status' where receipt_no='$receipt_no' ";
    }else{
         $queryAppi="update account_quotation set director_status='1', admin_remarks='$approve_remarks', admin_approved='$remarks_status' where receipt_no='$receipt_no' ";
    }
        $resulti = $mysqli->query($queryAppi) or die($mysqli->error.__LINE__);
        $resulti = $mysqli->affected_rows;
        $querysts="update account_requisition set hold_status='4' where receipt_no='$receipt_no' ";
        $resultstst = $mysqli->query($querysts) or die($mysqli->error.__LINE__);
        $resultstst = $mysqli->affected_rows;
