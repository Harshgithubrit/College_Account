...
$query="select mobile, email, user_name, user_id, date, time, remarks, category, priority, receipt_no, admin_approved, admin_remarks from account_quotation as a   group by a.receipt_no order by a.id DESC ";
...
if($result->num_rows > 0) {
	...
	while($row = $result->fetch_assoc()) {
	    ...
	    $queryi="select  sum(total) as total, sum(quantity) as quantity from account_quotation_item   where receipt_no='$rq' ";
