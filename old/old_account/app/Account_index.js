...
//  console.log(emp_id);
  ...
  $scope.view_employee =  function (index) {

    checkInternet();
  
    var emp_id = $scope.employee_details[index].emp_id;
  ...
  //  console.log(emp_id);
    ...
    id_edit = $scope.employee_details[index].id;
   url= "employee_profile.html?uid="+emp_id;
 ...
 $scope.onsearch = function () {
  ...
  //  alert($scope.searchemployee);
     ...
     // alert("Hi..");
      ...
      var seremp = "";
         if($scope.searchemployee == null || $scope.searchemployee == undefined || $scope.searchemployee == ""){
    
          seremp = "";
        }else{
          seremp = $scope.searchemployee ;
        }
  
          
      $scope.loading = true;
      
      var Query="select * from mj_staff where   emp_id LIKE '"+seremp+"%' or (firstname LIKE '"+seremp+"%'  or (mobile LIKE '"+seremp+"%')) order by id DESC LIMIT 50 ";
       console.log(Query);
      //  return;
       $scope.loading = true;
    
        $http.post("php/search_employee.php?search_query="+Query)
            .then(function (result) {
                $scope.employee_details = result.data;
              //  console.log(result.data);
                $scope.loading = false;
            }, function(result) {
                 $scope.loading = false;
...
function PurchaseData(){
  ...
  function getAllData(){  
    ...
    $scope.loading = true;
      ...
      var method="select";
      var query="select count(*) as tot_count from office_item_name";
    
    
      $http.post("php/API.php?query="+query+"&method="+method)
        .then(function (result) {
          console.log(result.data.tbldata);
             $scope.item_tot = result.data.tbldata[0].tot_count;
           // alert(result.data.tbldata);
    ...
    function getSupplierCount(){  
      ...
      $scope.loading = true;
        ...
        var method="select";
        var query="select count(*) as tot_sup from account_vendor";
      
      
        $http.post("php/API.php?query="+query+"&method="+method)
          .then(function (result) {
            console.log(result.data.tbldata);
               $scope.supplier_tot = result.data.tbldata[0].tot_sup;
             // alert(result.data.tbldata);
             console.log($scope.supplier_tot );
              // $scope.loading = false;
          }, function(result) {
               $scope.loading = false;
              //some error
      ...
      function getRequisitionCount(){  
        ...
        $scope.loading = true;
          ...
          var method="select";
          var query="select count(*) as tot_requi from account_requisition";
        
        
          $http.post("php/API.php?query="+query+"&method="+method)
            .then(function (result) {
              console.log(result.data.tbldata);
                 $scope.requisition_tot = result.data.tbldata[0].tot_requi;
               // alert(result.data.tbldata);
               console.log($scope.requisition_tot );
                // $scope.loading = false;
            }, function(result) {
                 $scope.loading = false;
                //some error
        ...
        function getQuotatationCount(){  
          ...
          $scope.loading = true;
            ...
            var method="select";
            var query="select count(*) as tot_quotation from account_quotation";
          
          
            $http.post("php/API.php?query="+query+"&method="+method)
              .then(function (result) {
                console.log(result.data.tbldata);
                   $scope.quotation_tot = result.data.tbldata[0].tot_quotation;
                 // alert(result.data.tbldata);
                 console.log($scope.quotation_tot );
                  // $scope.loading = false;
              }, function(result) {
                   $scope.loading = false;
                  //some error
