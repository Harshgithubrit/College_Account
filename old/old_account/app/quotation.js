...
var query=" select  vendor_id, vendor_name, mobile, email, user_name, user_id, date, time, remarks, category, priority, receipt_no, admin_approved, admin_remarks from account_quotation   group by receipt_no order by id DESC ";
...
function dialog_for_create_quotation(receipt_no_last, approve_remarks, remarks_status){
    
    
  console.log(receipt_no_last);
       
 $http.post("php/Approve_create_po.php?receipt_no="+receipt_no_last+"&approve_remarks="+approve_remarks+"&remarks_status="+remarks_status)
 .then(function (result) {
  console.log(result.data);
if(result.data['0'].status == "1") {
