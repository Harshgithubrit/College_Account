...
var query="select  vendor_id, vendor_name, mobile, email, user_name, user_id, date, time, remarks, category, priority, receipt_no, admin_approved, admin_remarks,delivery_date from account_purchase_order as a   group by a.receipt_no order by a.id DESC ";
...
//    console.log(index);
   ...
   console.log("notification_send ", rid);
    ...
    $scope.submit_cheque = function(){
      ...
      if($scope.cheque_pay == "" || $scope.cheque_pay == null || $scope.cheque_pay == undefined){

        swal("Please Enter an Amount");
        return;
      }

      if($scope.cheq_acc == "" || $scope.cheq_acc == null || $scope.cheq_acc == undefined){

        swal("Please Enter a Valid Account Number!");
        return;
      }

      if($scope.cheq_no_1 == "" || $scope.cheq_no_1 == null || $scope.cheq_no_1 == undefined){

        swal("Please Enter a Valid Cheque Number!");
        return;
      }

      if($scope.cheq_quant == "" || $scope.cheq_quant == null || $scope.cheq_quant == undefined){

        swal("Please Enter a Valid Cheque Number!");
        return;
      }

      if($scope.cheque_date == "" || $scope.cheque_date == null || $scope.cheque_date == undefined){
      ...
      if($scope.cheq_quant == "" || $scope.cheq_quant == null || $scope.cheq_quant == undefined){
        ...
        swal("Please Enter Number of Cheques you want to generate!");
        return;
      }
    
      var query = "INSERT INTO `cheque_master`( `ch_id`, `cheque_number`, `date`, `time`, `created_by`, `amount`, `cheque_date`, `account_number`, `amount_inwords`, `payee`) VALUES ( 'ch-03', '"+$scope.chq_no_1+"', now(), now(), '"+EMP_id+"', '"+$scope.cheque_pay+"', '"+$scope.cheque_date+"', '"+$scope.cheq_acc+"', '"+$scope.amnt_words+"', '"+$scope.to_pay+"' )";
  
      console.log(query);
  
      $http.post("php/gen_cheque.php?query="+query+"&method="+$scope.cheq_quant)
      .then(function (result) {
