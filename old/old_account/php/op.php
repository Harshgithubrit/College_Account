...
$query1="select receipt_no from account_purchase_order where receipt_no='$receipt_no' ";
...
$queryUpdater=" update `account_purchase_order` set `user_name`='$user_full_name', `create_date`='$requisition_date', `email`='$email', `mobile`='$phone',  `remarks`='$description', `category`='$category', `priority`='$priority' where receipt_no='$receipt_no' ";
 $mysqli->query($queryUpdater) or die($mysqli->error.__LINE__);

}else{

    $queryApp1=" INSERT INTO `account_purchase_order` 
    (`receipt_no`, `user_id`, `user_name`, `create_date`, `email`, `mobile`,  `remarks`,`category`,`priority`,`status`,`date`,`time`,`department`) 
    VALUES 
    ('$receipt_no','$user_id','$user_full_name','$requisition_date','$email','$phone','$description','$category','$priority','Active',now(),now(),'$department') ";
    ...
    $queryApp=" INSERT INTO `account_purchase_order_item` 
